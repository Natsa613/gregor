...
export default class dreamScene extends Phaser.Scene {
    constructor() {
        ...
        this.assets = [
            ...
            new Asset('littleGirlScreaming','./assets/sound/littleGirlScreming.mp3'),
            new Asset('knife','./assets/sound/knife.mp3'),
            new Asset('dreamMusic','./assets/sound/dream.mp3')
        ];
        this.content = null;
        this.textEvent = null;
    ...
    preload() {
        ...
        this.load.json('dreamData', './assets/text/dreamData.json'); // 텍스트, 이미지, 사운드 데이터 로드
    }

    create() {
        this.cameras.main.fadeIn(1000);
    
        const textData = this.cache.json.get('dreamData').contents; // Content.js에 맞춘 구조로 로드
        ...
        const images = {
            ...
            'dream': this.add.image(this.scale.width / 2, this.scale.height / 2, 'dream').setScale(1).setOrigin(0.5, 0.5).setVisible(true)
        };

        /*const sound = {
            'dreamMusic' : this.add.sound('dreamMusic',{loop:true}).setVolume(0.75),
            'littleGirlScreaming' : this.add.sound('littleGirlScreaming').setVolume(0.75),
            'knife' : this.add.sound('knife').setVolume(0.75)
        };*/
        this.dreamMusic = this.sound.add('dreamMusic',{loop:true}).setVolume(0.75);
        ...
        this.input.keyboard.on('keydown-SPACE', () => {
            ...
            if (!this.content.isComplete()) {
                ...
                if (this.content.moveToNext()) {
                    ...
                    this.scene.start('officeScene', { fadeIn: true }); // 다음 씬으로 이동
                }
            }
        });
