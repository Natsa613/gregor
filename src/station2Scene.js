...
export default class station2Scene extends Phaser.Scene{
    constructor(){
        super({key:'station2Scene'});
        this.assets =[
            ...
            new Asset('walk','./assets/sound/walk.mp3'),
            new Asset('house','./assets/introBackground.jpg')
        ];
        this.content = null;
        this.textEvent = null;
    ...
    preload(){
        ...
        this.load.json('station2Data', './assets/text/station2Data.json'); // 텍스트, 이미지, 사운드 데이터 로드
    }

    create(){
        this.cameras.main.fadeIn(1500);
    
        const textData = this.cache.json.get('station2Data').contents; // Content.js에 맞춘 구조로 로드
        ...
        const images = {
            ...
            'station': this.add.image(this.scale.width/2, this.scale.height/2, 'station').setScale(1).setOrigin(0.5, 0.5).setVisible(true).setDepth(2),
            'train': this.add.image(this.scale.width/2, this.scale.height/2, 'train').setScale(1).setOrigin(0.5, 0.5).setVisible(true).setDepth(2),
            'map': this.add.image(this.scale.width/2, this.scale.height/2, 'map').setScale(1).setOrigin(0.5, 0.5).setVisible(false).setDepth(9),
            'house': this.add.image(this.scale.width/2, this.scale.height/2, 'house').setScale(1).setOrigin(0.5, 0.5).setVisible(false).setDepth(2)
        };
        ...
        this.input.keyboard.on('keydown-SPACE', () => {
            ...
            if (!this.content.isComplete()) {
                ...
                if (this.content.moveToNext()) {
                    ...
                    this.scene.start('doorScene', { fadeIn: true }); // 다음 씬으로 이동
                }
            }
        });
        ...
        const skipButton = new Button(this, this.scale.width - 200, this.scale.height - 850, 'skipButton', 0.1, () => {
            this.scene.start('doorScene', { fadeIn: true });
        });
